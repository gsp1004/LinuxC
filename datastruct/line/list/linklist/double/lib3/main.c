...
int main(int argc, char *argv[])
{
	...
	for(int i=0;i<7;i++){
		...
		handler->insert(handler, &data, LLIST_BACKWARD);
	}

#if 1
	printf("********** show list ***********\n");
	printf("id\tname\tmath\tchinese\n");
	handler->travel(handler, callback_print);
	printf("********************************\n");
#else // demo to use llist_travel2
	if(0 != handler->travel2(handler, &data)){
		exit(1);
	}
	else{
		...
		while(0 == handler->travel2(NULL, &data));
	}
#endif
	...
	int id = 3;
	score * ret_data;
	ret_data = handler->find(handler, &id, id_cmp);
	if(NULL == ret_data)
	...
	id = 5;
	if(handler->delete(handler, &id, id_cmp))
	{
	...
	if(handler->delete(handler, delete_name, name_cmp))
	{
		DBGLINE("Can not delete\n");
	}
	else
	...
	id = 4;
	if(handler->fetch(handler, &id, id_cmp, &data))
	{
